...
% Introducci√≥n al Reconocimiento de Patrones Escuela de Ingenier√≠a
% Electr√≥nica Tecnol√≥gico de Costa Rica
%
% (C) 2021 <Su Copyright AQU√ç>
...
## Construct la matriz de pre diseÒo, o sea las columans con las 3 entradas: areas, pisos y cuartos
Xo=[D(:,1),D(:,2),D(:,3)];
...
## Normalize the data
  ...
  %%Creamos las matrices de diseÒo completas
  %para orden 1
  if (order=1)
    XX=[ones(rows(x),1) x];
    y=XX*theta;
  ...
  elseif (order=4)
    ...
    y=XX*theta;
  elseif (order>5)
    error("El lÌmite de orden es de 4");
  endif
